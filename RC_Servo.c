...
#define RC_SetPeriod(pin)   WritePeriod3(ReadTimer3() + uSEC * RCupTime[pin])
 ...
 * @brief Initialize the RC module for use. Uses TIMER3 hardware and initializes
...
char RC_Init(void)
{
    ...
    // Set up Timer #3 to overflow at more than 2msec, enable and set interrupt
    OpenTimer3(T3_ON | T3_IDLE_STOP | T3_GATE_OFF | T3_PS_1_2 | T3_SOURCE_INT, SERVOCENTER * uSEC);
    // Initialize the interrupt module
    INTClearFlag(INT_T3);
    INTSetVectorPriority(INT_TIMER_3_VECTOR, 3);
    INTSetVectorSubPriority(INT_TIMER_3_VECTOR, 0);
 ...
 *        next rollover of TIMER3 (20msec max). Will return ERROR if
 ...
 *        down on the next rollover of TIMER3 (20msec max). Function will
 ...
 * @param none
 * @return SUCCESS or ERROR
 * @brief Shuts down the RC_SERVO module. Sets flag so that TIMER3 will
...
char RC_End(void)
{
    ...
    // T3 rollover unless SHUTDOWN_WITH_SHORT_PULSE is defined, in which case
...
/**
 * @Function Timer3IntHandler -- ISR ROUTINE
 * @param none
 * @return none
 * @brief Implements the RC_Servo state machine through the TIMER3 ISR.
...
void __ISR(_TIMER_3_VECTOR, ipl4) Timer3IntHandler(void)
{
    ...
    case none:
        ...
        WritePeriod3(ReadTimer3() + uSEC * SERVOCENTER);
        if (pinsToAdd) {
